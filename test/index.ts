...
describe('Run demo', (): void => {
  it('Should get one result as Array', async (): Promise<void> => {
    const result: DetailType[] | null = await esliteComCollection('春夏秋冬，日日食光。生活美學家的鑄鐵鍋料理手帖');
  ...
  it('Should do not have any result as Null', async (): Promise<void> => {
    const result: DetailType[] | null = await esliteComCollection('blablablablablablablablablablablabla');
